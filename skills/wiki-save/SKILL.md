...
Close the session by filing **THIS session's** work into durable knowledge, **right here (warm context)**, and keep the **reading input (L0) fresh**. This is the primary integration path — warm, human-present, not an unattended cron. **Minutes, not tens of minutes**: close-out latency is what makes people abandon a wiki (the maintenance-burden rule), so this pass is strictly O(this session). Everything heavier — the transcript backlog, the gap queue, topic re-distillation — belongs to the periodic **`/wiki-deep`** deep pass (the semantic review is the one exception: the close-out *launches* it in the background behind the engine's cadence gate, step 8, precisely so it never adds wall-clock here), and **deferring it is safe within the harness's retention window** (Claude Code deletes its transcripts on `cleanupPeriodDays`, default 30; Codex keeps them): the capture watermarks, gap queue, and lint backlog are durable, so everything deferred stays queued — but the transcript itself is not immutable, and once the harness deletes it that session can no longer be filed. An un-filed session that ages out is **self-selection, not loss** — the human judged it not worth keeping (2026-07-28: for daily work the "expiring soon" band is perpetually non-empty, so the engine never pushes deadlines; retention detail is on pull only, via `llmwiki doctor`).
...
- **This is the write moment**: cold-start rule 3 defers ALL mid-session page-writing to this close-out, so sweep the WHOLE session — the decision made in its first hour was deliberately left unwritten then and lands here, not just the last thing worked on.
- **Standalone CLI engine** — only `bun "<plugin-root>/src/cli.ts"` + `<repo>/docs/wiki/`. Don't confuse it with other wiki/MCP tools.
...
1. **REPO = the repo this session's WORK concerned** — normally cwd (`$CLAUDE_PROJECT_DIR`; argument none/`here` = current repo, or pass another repo's path). A session that ran from an enrolled home or any other catch-basin cwd while its edits went to a different enrolled repo files into THAT repo — you did the work warm, you know where it happened, and `update-next` prints a deterministic `# touched:`/`# ⚠ route:` line for the cold case. If `docs/wiki/` is missing, create it with `llmwiki skeleton <repo>` then proceed.
...
2. **File THIS session into the log** — current-session selection is **EXACT, never nearest**:
   - **Resolve the current session id** (harness-provided, deterministic — never inferred from recency):
   ...
   - Run `llmwiki save-current <repo> --session <that id>` — it resolves THIS session's transcript(s) by exact identity, enqueues them (manual save: the daemon's 50-line auto-capture threshold does NOT apply here), and prints their exact paths.
   - **If save-current fails (no exact match): file NOTHING for this session.** Report the failure in the close-out summary and continue with the remaining steps. Never fall back to the newest pending entry or any other session's transcript — a recency pick can select a different session or even a different harness and record someone else's work as this session's judgment.
   - `llmwiki update-status <repo>` → from the pending list take **only the path(s) save-current printed** → for each, `llmwiki update-next <repo> <transcript>` (judge from the extract alone — never load the full transcript into context; `update-next` prints a harness-summary block when one exists — use it as draft material but ground every claim in the raw extract). **Route check first**: the extract header's `# touched:` line lists the git roots this segment's file mutations landed in, and a `# ⚠ route:` advisory appears when the dominant root is NOT this repo — then file the session into the dominant repo's wiki (this procedure, with that repo as REPO) and afterwards advance the watermark where the transcript is actually queued: `llmwiki update-done <bucket-repo> <transcript> <offset>`. The `cwd=` field cannot catch this — capture buckets by session cwd, so bucket and cwd agree even when every edit went elsewhere. From the extract, pick out **only what the human actually said/decided/realized**; discard model-generated analysis. Route by category (create a page only for categories that have content):
...
2b. **Same-topic weave-in (optional, zero pressure)**: run `llmwiki related <repo> <this session's transcript>` — deterministic (~0.5s on a 60-session backlog, no LLM), it matches the HUMAN's own utterances only (injected hook text and harness-fed user-role text like skill bodies are excluded by construction). If it lists candidates, mention them once. Weave one in ONLY when it would enrich a page this close-out is already writing (same topic page, same decision) — via the normal `update-next → file → update-done` flow, each woven session getting its own log entry. **Skipping is the default and is final for today** (self-selection): the list is candidates, never debt, and never a reason to recommend `/wiki-deep`.
...
4. **Refresh L0 freshness** (the reading input — most important):
   ...
   - **Trim to the budget while you are here (~1,600 chars)**: L0 is injected whole at every cold start, so an over-budget L0 is a per-session tax the cold-start banner keeps flagging until someone acts — and "someone" is THIS step, because invoking the close-out is the moment the human has already accepted upkeep latency (2026-08-05 direction). Move displaced detail to the paired detail page (`current-state-detail.md`, create it beside L0 if absent, link it from L0) — **move, never delete**: cutting content instead of relocating it silently discards the handoff. Keep only 'now'+'next' in L0 itself.
...
7. **Close out (deterministic)**: `llmwiki register-transcript <repo>` → `llmwiki index <repo>` → `llmwiki reconcile <repo> --commit` → `llmwiki lint <repo> --errors-only` → `llmwiki daemon-sync`. The last is engine hygiene, one line, always exit 0: it restarts the capture daemon only when it predates the code on disk (observed 2026-08-21: a live daemon ran the day's fixes 209 minutes late because restarting relied on a human remembering launchctl). It rides here because the close-out is where upkeep latency is already consented to — never fail the close-out over it. Thanks to validate-as-you-write this should **confirm** 0 errors, not open a repair phase; fix any residue until **0 errors**. Warnings arrive collapsed to per-code counts — report the counts as advisory backlog, don't over-format to silence them.
...
9. **Report** (1–2 lines): "log reflected N (this session) / **backlog deferred: B sessions** / **topic pages: created A, updated B** / **gaps filled K (open M)** / 0_review pending N / L0 refreshed: yes/no / lint error·warn counts / review: launched-bg|not-due|prev-incomplete / maintenance: no-action|db <databaseBytes>B free <freeBytes>B ratio <freeRatio> → `/wiki-deep` / **deep pass (`/wiki-deep`) recommended: yes/no (why)**". Recommend `/wiki-deep` only for WIKI-maintenance signals: lint shows `topic-oversize` · open gaps keep being deferred · the maintenance notice emits threshold guidance. The pending-backlog count is NEVER a reason to recommend it — un-filed past sessions are self-selection (2026-07-28), report the count as a bare fact and move on.
