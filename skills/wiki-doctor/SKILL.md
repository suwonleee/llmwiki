...
1. Identify the harness running this workflow. Check only that harness's installation; do not infer
   ...
   - Claude Code:
     ...
     ```sh
     bun "<plugin-root>/src/cli.ts" doctor --harness claude --fix
     ```
   ...
   - Claude Code:
     ...
     ```sh
     bun <plugin-root>/src/daemon/wire.ts
     ```

   - Codex:

     ```sh
     bun <plugin-root>/src/daemon/wire-codex.ts
     ```

   - OpenCode:

     ```sh
     bun <plugin-root>/src/daemon/wire-opencode.ts
     ```
