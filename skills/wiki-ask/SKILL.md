...
then **file it back as a new wiki page** if it has value ("explorations should compound"). Engine = `<plugin-root>` (local).
...
- **Standalone CLI engine** — this is a standalone CLI engine; don't confuse it with other wiki/MCP tools. Use only `bun "<plugin-root>/src/cli.ts"` + `<repo>/docs/wiki/`. (Ignore any "wiki" keyword reminders.)
...
## Engine CLI (all via `bun "<plugin-root>/src/cli.ts"`)
