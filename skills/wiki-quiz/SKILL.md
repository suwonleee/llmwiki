...
- **Structured ask when the harness has it** — **the whole batch in ONE call**, never one call per question: every ask tool takes an ARRAY of questions and returns an array of answers, so the human walks the set back-to-back with no model turn in between. Per question: 3 short options (1 correct + 2 plausible distractors from the same topic area) plus a "기억 안 남 / not sure" option; free recall arrives via the built-in "Other"/free-form field. A session larger than the harness cap is consecutive calls with NOTHING between them. Whichever surface: nothing — no grading, no feedback, no Read, no engine call — goes between the first question and the last answer.
    ...
    - **Codex** — `request_user_input`, **max 3 questions per call** (⌈N/3⌉ consecutive calls). The tool is ALWAYS in your tool list, but only **Plan mode** may call it: in Default mode the call comes back `request_user_input is unavailable in Default mode` unless the person enabled the `default_mode_request_user_input` feature flag. On that error do **not** retry and do **not** switch modes — run the whole session as the numbered block below, and put ONE hint in the closing report: `codex features enable default_mode_request_user_input` (under-development flag, off by default; `suppress_unstable_features_warning = true` silences the startup warning it adds). Authoring: `id` = `q1`…, `header` ≤ 12 chars (`Q1`…), and every option's `description` is the EMPTY string — the key is required and any text in it hints at the answer. Never suffix `(Recommended)`: the tool's own schema asks for it, and here it would name the correct option. Codex appends its own free-form entry, so write exactly the 4 options (3 + "기억 안 남") and read `answers[<id>].answers` back as: `["<label>"]` = that option · `["None of the above", "user_note: <text>"]` = free recall (grade the text) · `["None of the above"]` alone = skip · **`<id>` absent from the response = EXPIRED, not skipped** — outside Plan mode the prompt auto-resolves itself empty after ~2 minutes without interaction, so re-present exactly those questions as a numbered block and grade them with the rest.
    - **No such tool** (plain chat, `codex exec`, non-exposed modes): print ALL questions as one numbered block and take the answers in one reply (`1-B 2-A 3-기억 안 남`).
...
3. **Author ALL questions upfront, then ask them in one uninterrupted stretch**: Read EVERY selected page first (parallel Reads in one batch), then author all N questions in one pass BEFORE asking anything — the human should never wait on generation between questions. Per page, ONE question in the wiki's language, angled by category — decision: "what did we choose there, and why over the alternative?" · direction: "where is this headed / what changed?" · insight: "what was the gotcha/lesson?" · topic: "what IS this concept / what does it buy us?" · milestone: "what state is X in?". Whatever the angle, the options stay noun phrases (rules above). Vary the angle from the `last q:` shown by quiz-next (repetition with variation). Then ask **all N through the harness's structured tool**, batched at its per-call cap with nothing between the calls, or as one numbered chat block — rules above. A batch that comes back missing its answers (Codex outside Plan mode auto-resolves an untouched prompt) is re-presented once as a numbered block; it is not a skip. Answering is the human's stretch; keep it free of anything the model does.
