...
- **Structured ask when the harness has it** — **the whole session in ONE call**, never one call per question: every ask tool takes an ARRAY of questions and returns an array of answers, so the human walks the set back-to-back with no model turn in between. Per question: 3 short options (1 correct + 2 plausible distractors from the same topic area) plus a "기억 안 남 / not sure" option; free recall arrives via the built-in "Other"/free-form field. Claude Code: `AskUserQuestion` — **caps at 4 questions per call**, so a 5–7 question session is two consecutive calls with nothing between them. OpenCode: the `question` tool (available in the TUI; it appends "Type your own answer" itself — don't author one). Codex: `request_user_input` (exposed in Plan mode; in Default mode only behind the `default_mode_request_user_input` feature flag — if the tool is not in your tool list, fall back below). Harnesses without such a tool (plain chat, non-exposed modes): print ALL questions as one numbered block and take the answers in one reply (`1-B 2-A 3-기억 안 남`). Whichever surface: nothing — no grading, no feedback, no Read, no engine call — goes between the first question and the last answer.
...
3. **Author ALL questions upfront, then ask them in one uninterrupted stretch**: Read EVERY selected page first (parallel Reads in one batch), then author all N questions in one pass BEFORE asking anything — the human should never wait on generation between questions. Per page, ONE question in the wiki's language, angled by category — decision: "what did we choose there, and why over the alternative?" · direction: "where is this headed / what changed?" · insight: "what was the gotcha/lesson?" · topic: "what IS this concept / what does it buy us?" · milestone: "what state is X in?". Whatever the angle, the options stay noun phrases (rules above). Vary the angle from the `last q:` shown by quiz-next (repetition with variation). Then ask **all N in a single structured call** (Claude Code: two consecutive calls when N > 4, nothing between them) or as one numbered chat block — rules above. Answering is the human's stretch; keep it free of anything the model does.
