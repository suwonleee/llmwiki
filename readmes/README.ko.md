...
개발자 1명, 머신 1대, 레포 19개, 페이지 997개 — 2026-08-21 실측이며 추정치가 아닙니다.
아래는 모두 엔진이 스스로에 대해 보고하는 수치로, `llmwiki doctor` · `llmwiki bench` ·
`llmwiki downstream-read` 로 본인 위키에서 그대로 확인할 수 있습니다.

| 매일 쓰며 관찰한 것 | 실측 | 왜 중요한가 |
| --- | --- | --- |
| 세션당 비용이 위키 크기를 따라 늘지 않는다 | 가장 큰 레포(236페이지)에서 세션 시작 주입 6.7KB | 포인터 수가 비례가 아니라 상한제 — 위키가 커져도 세션 세금은 같이 커지지 않는다 |
| 검색이 대화 속도를 유지한다 | 같은 레포에서 검색 0.11초 · 턴 훅 0.16초 | 매 프롬프트마다 도는 루프라, 이보다 느려지면 바로 체감된다 |
| 파생 데이터는 다시 만들면 그만 | 전량 재색인 6.9초 | 위키 본체는 git 이력 속 평범한 마크다운이고, 인덱스는 버려도 되는 사본 |
| 로컬 상태가 무한 증가하지 않는다 | 위키 19개 합쳐 167MB | 캡처가 하네스 자체의 보관 주기와 함께 만료된다 |
| 위키 하나를 여러 에이전트가 읽는다 | 같은 레포에 대해 Codex·Claude Code 양쪽에서 열람 기록 | 에이전트를 바꿔도 지식이 처음부터 다시 시작되지 않는다 |
| 전달 효과를 낮을 때까지 포함해 측정한다 | 포인터 열람률 2.1% | 가정하지 않고 보고한다 — 같은 계기가 자기 표본 결함(54세션 중 5개만 읽고 낸 "0%")도 잡아냈다 |

한 사람이 한 대의 머신에서 11주간, 본인이 쓰는 언어의 위키로 낸 수치입니다. 이 규모에서 엔진이
가볍고 정직하게 동작한다는 뜻이지, 팀 단위·다른 코퍼스·답변 품질에 대한 주장은 아닙니다.
...
새 버전이 있으면 다음 세션 시작 시 한 줄 알림과 적용 명령이 표시됩니다. 이 알림은 에이전트만이
아니라 **사용자에게** 보입니다 — Claude Code·Codex는 세션 시작 시 훅 시스템 메시지로, OpenCode는
토스트로 띄우고, 모든 `llmwiki` 명령이 stderr에 한 번 출력합니다. 버전 번호는 같은데 origin의
코드가 더 새로운 경우도 알립니다. 실행은 언제나 사용자의
