...
開発者1名、マシン1台、リポジトリ19個、ページ997件 — 2026-08-21の実測であり、推計ではありません。
以下はすべてエンジンが自分自身について報告する数値で、`llmwiki doctor`・`llmwiki bench`・
`llmwiki downstream-read` により自分のウィキでそのまま確認できます。

| 毎日の利用で観察したこと | 実測 | なぜ重要か |
| --- | --- | --- |
| セッションあたりのコストがウィキの大きさに比例しない | 最大のリポジトリ（236ページ）でセッション開始時の注入6.7KB | ポインタ数は比例ではなく上限制 — ウィキが育ってもセッションの税は一緒に育たない |
| 検索が対話の速度を保つ | 同じリポジトリで検索0.11秒・ターンフック0.16秒 | 毎プロンプト走るループなので、これより遅いと必ず体感される |
| 派生データは作り直せば済む | 全件再インデックス6.9秒 | ウィキ本体はgit履歴の中の素のMarkdownで、インデックスは捨ててよい写し |
| ローカル状態が無限に増えない | ウィキ19個で合計167MB | キャプチャがハーネス自身の保持期間とともに期限切れになる |
| 一つのウィキを複数のエージェントが読む | 同じリポジトリに対しCodexとClaude Codeの双方から閲覧記録 | エージェントを替えても知識がゼロから始まらない |
| 効果が低いときも含めて計測する | ポインタ開封率2.1% | 仮定せず報告する — 同じ計器が自身の標本欠陥（54セッション中5件だけを読んで出した「0%」）も検出した |

一人の開発者が一台のマシンで11週間、本人が書く言語のウィキで得た数値です。この規模でエンジンが
軽く正直に動くことを示すもので、チーム運用・他のコーパス・回答品質についての主張ではありません。
...
新しいバージョンがあれば次のセッション開始時に1行の通知と適用コマンドが表示されます。この通知は
エージェントだけでなく**ユーザー本人**に見えます — Claude CodeとCodexではセッション開始時のフック
システムメッセージとして、OpenCodeではトーストとして表示され、すべての`llmwiki`コマンドがstderrに
1回出力します。バージョン番号が同じままoriginのコードだけ新しい場合も通知します。実行するのは
