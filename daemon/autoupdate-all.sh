...
# (synthesize) and judgment are JUDGMENT — they stay WARM (run /wiki-save, or the /wiki-deep
...
for repo in $REPOS; do
    ...
    # `/wiki-save` (or the `/wiki-deep` deep pass) in a human-present session for that.
    "$(command -v bun)" "$ROOT/src/cli.ts" autoupdate "$repo" $COMMIT --limit "$PER" 2>&1 | sed 's/^/  /'
done
echo "### done (facts only). review: <repo>/docs/wiki/0_review/ — spot-check. 통합은 웜 /wiki-save·/wiki-deep. ###"
