...
export function parseQueue(md: string): Gap[] {
  ...
  for (const line of md.split("\n")) {
    const m = line.match(
      /^\s*-\s*\[(x| )\]\s*\((missing-concept|next-question)\)\s*(.*?)\s*<!--\s*gap:([0-9a-f]+)\s+absent:(\d+)\s+seen:([0-9-]+)\.\.([0-9-]+)\s*-->/i,
    );
    if (!m) continue;
    out.push({
      status: m[1] === "x" ? "resolved" : "open",
      type: m[2]!,
      text: m[3]!.replace(/—\s*covered.*$/, "").trim(),
      hash: m[4]!,
...
export function renderQueue(gaps: Gap[], date: string): string {
  ...
  const row = (g: Gap) =>
    ...
    `  <!-- gap:${g.hash} absent:${g.absent} seen:${g.firstSeen}..${g.lastSeen} -->`;
  const fm =
  ...
  return (
    ...
    (resolved.length ? resolved.map(row).join("\n") : "(none)") +
    "\n"
  );
}
...
export function refreshGapQueue(ws: string, date: string, opts: { check?: boolean } = {}): GapResult {
  ...
  const prior = existsSync(queuePath) ? parseQueue(readFileSync(queuePath, "utf-8")) : [];
  ...
  for (const g of byHash.values()) {
    ...
    g.absent += 1;
    if (g.absent >= RESOLVE_AFTER) g.status = "resolved";
  }
